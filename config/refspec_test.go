...
func (s *RefSpecSuite) TestRefSpecMatch() {
	spec := RefSpec("refs/heads/master:refs/remotes/origin/master")
	s.Equal(false, spec.Match(plumbing.ReferenceName("refs/heads/foo")))
	s.Equal(true, spec.Match(plumbing.ReferenceName("refs/heads/master")))
	...
	spec = RefSpec(":refs/heads/master")
	s.Equal(true, spec.Match(plumbing.ReferenceName("")))
	s.Equal(false, spec.Match(plumbing.ReferenceName("refs/heads/master")))

	spec = RefSpec("refs/heads/love+hate:heads/love+hate")
	s.Equal(true, spec.Match(plumbing.ReferenceName("refs/heads/love+hate")))
...
func (s *RefSpecSuite) TestRefSpecReverse() {
	spec := RefSpec("refs/heads/*:refs/remotes/origin/*")
	s.Equal(RefSpec("refs/remotes/origin/*:refs/heads/*"),
		spec.Reverse(),
	)
}
