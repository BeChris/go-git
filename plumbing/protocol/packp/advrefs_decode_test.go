...
func (s *AdvRefsDecodeSuite) testDecoderErrorMatches(input io.Reader, pattern string) {
	ar := NewAdvRefs()
	s.ErrorContains(ar.Decode(input), pattern)
}
...
func (s *AdvRefsDecodeSuite) TestHead() {
	...
	ar := s.testDecodeOK(payloads)
	s.Equal(plumbing.NewHash("6ecf0ef2c2dffb796033e5a02219af86ec6584e5"), *ar.Head)
}
...
func (s *AdvRefsDecodeSuite) TestFirstIsNotHead() {
	...
	ar := s.testDecodeOK(payloads)
	s.Nil(ar.Head)
	s.Equal(plumbing.NewHash("6ecf0ef2c2dffb796033e5a02219af86ec6584e5"), ar.References["refs/heads/master"])
}
...
func (s *AdvRefsDecodeSuite) TestWithPrefixAndFlush() {
	...
	s.Len(ar.Prefix, 2)
	s.Equal([]byte("# this is a prefix"), ar.Prefix[0])
	s.Equal([]byte(pktline.FlushString), ar.Prefix[1])
}
